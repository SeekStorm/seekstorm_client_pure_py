"""End-to-end tests for the pure Python SeekStorm REST client.
...
class TestSeekStormClientE2E(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.client = SeekStorm(base_url=BASE_URL, apikey_base64=DEMO_API_KEY)
        try:
            live = cls.client.live()
        except Exception as exc:  # pragma: no cover - runtime environment dependent
            raise unittest.SkipTest(f"SeekStorm server not reachable at {BASE_URL}: {exc}")

        if "SeekStorm" not in live.message:
            raise unittest.SkipTest(f"Unexpected live response: {live.message}")
    ...
    def _create_test_index(self):
        request = CreateIndexRequest(
            index_name=f"e2e_{self._testMethodName}_{int(time.time() * 1000)}",
        ...
        return self.client.create_index(request).index_id

    def test_10_live(self):
        result = self.client.live()
        self.assertIn("SeekStorm", result.message)

    def test_20_create_and_delete_apikey_optional(self):
        ...
        created = self.client.create_apikey(MASTER_API_KEY, quota)
        self.assertTrue(created.api_key_base64)

        info = self.client.get_apikey_info(apikey_base64=created.api_key_base64)
        self.assertIsNotNone(info.indices)

        deleted = self.client.delete_apikey(created.api_key_base64, MASTER_API_KEY)
        self.assertGreaterEqual(deleted.remaining_api_keys, 0)

    def test_30_index_lifecycle_end_to_end(self):
        index_id = self._create_test_index()
        self.assertGreaterEqual(index_id, 0)

        try:
            info = self.client.get_index_info(index_id)
            ...
            index_single = self.client.index_document(index_id, one_doc)
            ...
            index_bulk = self.client.index_documents(index_id, many_docs)
            self.assertGreaterEqual(index_bulk.indexed_document_count, 0)

            committed = self.client.commit_index(index_id)
            ...
            query_result = self.client.query_index(index_id, query_request)
            ...
            iterator_result = self.client.document_iterator(index_id, iterator_request)
            ...
            got_doc = self.client.get_document(index_id, doc_id, get_doc_request)
            ...
            deleted_single = self.client.delete_document_by_docid(index_id, doc_id)
            ...
            self.client.index_document(index_id, marker_doc)
            self.client.commit_index(index_id)

            delete_query = SearchRequestObject(query_string="+delete-query-marker", length=10)
            deleted_by_query = self.client.delete_documents_by_query(index_id, delete_query)
            self.assertGreaterEqual(deleted_by_query.indexed_document_count, 0)

            cleared = self.client.clear_index(index_id)
